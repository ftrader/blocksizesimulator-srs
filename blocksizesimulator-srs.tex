...
\subsubsection{BLOCKSIM-NODE-MINING-REQ-2 (mining node soft cap)}

Mining nodes will have a `soft cap' attribute configured by default to
1/4 of the node's hard cap, and which will limit the size of blocks produced
by the mining node.


\subsubsection{BLOCKSIM-NODE-MINING-REQ-3 (meaning of zero soft cap)}

A value of zero (0) for `soft cap' parameter shall mean that the mining
node can produce blocks as big as its its hard limit.


\subsubsection{BLOCKSIM-NODE-MINING-REQ-4 (fill to soft cap unless zero)}

A mining node will attempt to fit in as many transactions from its mempool
as possible without exceeding its soft cap if that is zero.

NOTE: a soft cap of 0 means fill to the hard cap.


\subsubsection{BLOCKSIM-NODE-MINING-REQ-5 (traffic agnostic)}

A mining node shall not discriminate between `legit' and `spam' transactions
when filling a block.

\subsubsection{BLOCKSIM-NODE-MINING-REQ-6 (minfee for mining)}

Transaction which do not carry a minimum fee rates as expected by a mining
node shall not mined by that mining node.


\subsubsection{BLOCKSIM-NODE-MINING-REQ-6 (drop arriving below minfee txs)}
...
Transactions have a size and associated fee (collected by the miner).
...
\subsubsection{BLOCKSIM-TRAFFICGEN-REQ-3 (generated transaction type)}
